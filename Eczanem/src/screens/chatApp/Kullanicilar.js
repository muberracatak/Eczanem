    ...
    const chateGit = () => {
        {
            user.map((kisi) => (
                ...
                navigation.navigate('ChatDeneme', { userId: kisi.userId, userMail: kisi.email })
            ))
        }
    }
    return (
        <View style={styles.container}>
            {user.map((kisi) => (
                <TouchableOpacity onPress={() => navigation.navigate('ChatDeneme', { userMail: kisi.email, userId: kisi.userId })} style={styles.eczaciContainer} key={kisi.userId}>
                    <View style={styles.avatarContainer}>
                        {/* <Text style={styles.avatarText}>{eczaci.firstName.charAt(0)}</Text> */}
                    </View>
                    <TouchableOpacity style={styles.infoContainer} >
                        <Text style={styles.adText}>{kisi.pharmacyName}</Text>
